...
if (cmd == "kill"){
	if(!args[0] || message.mentions.members.first() == undefined){message.channel.send("❌ You need to mention a valid user to kill")}
		...
		if (way == waylist[0] && part == partlist[0]) {
			...
			let resultlist = [`But **${user}** used a healthpack!`, `But **${user}** pressed X and was healed.`, `**${user}** died, but was resurrected!`, `But **${user}** was rushed into hospital and saved.`, `But **${user}** decided they didn't want to die yet and just left.`, `**${user}** couldn't handle it and ran away, then falling off a cliff.`, `R.I.P. **${user}** :skull:`, `**${user}** died from emotional despair.`, `**${user}** succumbed to his injuries.`, `**${user}** died on the spot.`]
